...
#######################################################################################################################
    ...
    def _gen_embed(self):
        ...
        e = discord.Embed()
        e.colour = 8311585
        e.set_footer(text='owo ({})'.format(BOTVERSION), icon_url='https://media1.tenor.com/images/c111b2c2878c35b50c1aec999a581ee8/tenor.gif?itemid=18044656')
    ...
    async def cmd_help(self, message, channel, command=None):
        ...
        desc = '```\n' + ', '.join(self.commands) + '\n```\n' + self.str.get(
            'cmd-help-response', 'For information about a particular command, run {}help [command].`\n'
                                 'For any help contact Princess Hamburbur#3431').format(prefix)
        if not self.is_all:
            desc += self.str.get('cmd-help-all', '\nOnly showing commands you can use, for a list of all commands you can run, . `{}help all`').format(prefix)
    ...
    async def cmd_summon(self, channel, guild, author, voice_channel):
        ...
        return Response(self.str.get('cmd-summon-reply', 'Connected to `{0.name}`').format(author.voice.channel), delete_after=35)
    ...
    async def cmd_pet(self, author, user_mentions):
        ...
        """
        if not user_mentions:
            return Response(self.str.get('cmd-pet-response', 'you cant pet yourself, you silly potato :P').format(author.id), reply=True, delete_after=75)
        else:
            usr = user_mentions[0]
            return Response(self.str.get('cmd-pet-response-other', 'has petted **{0}** UwU').format(usr.name, usr.id), reply=True, delete_after=75)

    async def cmd_cuddle(self, author, user_mentions):
        """
        Usage:
            {command_prefix}cuddle

        Cuddles either player if given their @ or cuddles auther of command
        """
        if not user_mentions:
            return Response(self.str.get('cmd-cuddle-response', 'cuddles with **GoodDragon**!').format(author.id), reply=True, delete_after=75)
        else:
            usr = user_mentions[0]
            return Response(self.str.get('cmd-cuddle-response-other', 'cuddles with **{0}**! How cuteee <3').format(usr.name, usr.id), reply=True, delete_after=75)

    async def cmd_snuggle(self, author, user_mentions):
        """
        Usage:
            {command_prefix}snuggle

        snuggle either a player or the bot when not providing their @
        """
        if not user_mentions:
            return Response(self.str.get('cmd-snuggle-response', 'snuggles with **GoodDragon**!').format(author.id), reply=True, delete_after=75)
        else:
            usr = user_mentions[0]
            return Response(self.str.get('cmd-snuggle-response-other', 'snuggles with **{0}**! How cuteee <3').format(usr.name, usr.id), reply=True, delete_after=75)

    async def cmd_boop(self, author, user_mentions):
        """
        Usage:
            {command_prefix}boop

        boop either a player or the bot when not providing their @
        """
        if not user_mentions:
            return Response(self.str.get('cmd-boop-response', 'boops **GoodDragon**!').format(author.id), reply=True, delete_after=75)
        else:
            usr = user_mentions[0]
            return Response(self.str.get('cmd-boop-response-other', 'boops **{0}**! How cuteee :3').format(usr.name, usr.id), reply=True, delete_after=75)
    ...
    async def cmd_hug(self, author, user_mentions):
        ...
        hug either a player or the bot when not providing their @
        """
        if not user_mentions:
            return Response(self.str.get('cmd-hug-response', 'got hugged by **GoodDragon**!').format(author.id), reply=True, delete_after=75)
        else:
            usr = user_mentions[0]
            return Response(self.str.get('cmd-hug-response-other', 'tightly hugs **{0}**! How cuteee <3').format(usr.name, usr.id), reply=True, delete_after=75)

    async def cmd_pounce(self, author, user_mentions):
        """
        Usage:
            {command_prefix}pounce

        pounce on a user or the bot when not providing their @
        """
        if not user_mentions:
            return Response(self.str.get('cmd-pounce-response', 'pounces on **GoodDragon**! how cute').format(author.id), reply=True, delete_after=75)
        else:
            usr = user_mentions[0]
            return Response(self.str.get('cmd-pounce-response-other', 'pounces on **{0}** and they are both happy! how cute.').format(usr.name, usr.id), reply=True, delete_after=75)

    async def cmd_coinflip(self, author, user_mentions, channel):
        """
        Usage:
        ...
        variable = [
        "heads", "tails",]
        await self.safe_send_message(channel, "{}".format(random.choice(variable)), reply=True)

    async def cmd_ping(self, channel):
        await self.safe_send_message(channel, "Pong! 🏓 | {}ms".format(self.latency), reply=True)

    async def cmd_pride(self, channel):
        """
        Usage:
            {command_prefix}pride

        sends a lgbtq gif
        """
        var = [
        "https://tenor.com/view/pride-pride-flag-flags-rainbow-lgbtq-gif-17886972",
        "https://tenor.com/view/be-supportive-pride-flag-pride-rainbow-lgbt-gif-17605761",
        "https://tenor.com/view/gay-gaylove-loveislove-gaypride-pride-gif-9826681",]
        await self.safe_send_message(channel, "{}".format(random.choice(var)), reply=True)

    async def cmd_flag(self, channel):
        """
        Usage:
             {command_prefix}flag

        send the lgbtq flag!
        theres nothing to descriminate just be you!
        """
        await self.safe_send_message(channel, ":rainbow_flag:")


    async def cmd_potato(self, channel):
        """
        Usage:
            {command_prefix}potato

        sends a picture of a potato
        """
        var = [
        "https://encrypted-tbn0.gstatic.com/images?q=tbn%3AANd9GcRG_CIJPpXPx1WEgGFuQg7HIDXKreULO2TJ4A&usqp=CAU",
        "https://encrypted-tbn0.gstatic.com/images?q=tbn%3AANd9GcSnoS0dxYEjP1vMluCIjE7qXvf0NyXeZemY-A&usqp=CAU",
        "https://encrypted-tbn0.gstatic.com/images?q=tbn%3AANd9GcTIJzRvRf2M2J0hHykuhg3i1pih8xQHhtjhYQ&usqp=CAU",
        "https://www.irishtimes.com/polopoly_fs/1.3967277.1564062363!/image/image.jpg_gen/derivatives/box_620_330/image.jpg",
        "Potato!", ]
        await self.safe_send_message(channel, "{}".format(random.choice(var)), reply=True)

    async def cmd_jakeyboi(self, channel, message, author):
        if message.channel.id == 738977422581694607:
                return Response(self.str.get('Jakes-Tik-Tok', 'Sorry unable to send message').format(author.id), reply=True, delete_after=60)
        else: 
            if message.channel.id == 739163145901310013:
                return Response(self.str.get('Jakes-Tik-Tok', 'Sorry unable to send message').format(author.id), reply=True, delete_after=60)
            else:
                if message.channel.id == 738981082921173085:
                    return Response(self.str.get('Jakes-Tik-Tok', 'Sorry unable to send message').format(author.id), reply=True, delete_after=60)
                else: 
                    if message.channel.id == 728696428997640265:
                        return Response(self.str.get('Jakes-Tik-Tok', 'Sorry unable to send message').format(author.id), reply=True, delete_after=60)
                    else:
                        if message.channel.id == 739157372362293329:
                            return Response(self.str.get('Jakes-Tik-Tok', 'Sorry unable to send message').format(author.id), reply=True, delete_after=60)
                        else: 
                            if message.channel.id == 739022711317200948:
                                return Response(self.str.get('Jakes-Tik-Tok', 'Sorry unable to send message').format(author.id), reply=True, delete_after=60)
                            else:
                                if message.channel.id == 739078501252202536:
                                    return Response(self.str.get('Jakes-Tik-Tok', 'Sorry unable to send message').format(author.id), reply=True, delete_after=60)
                                else:
                                    if message.channel.id == 739190590608113664:
                                        return Response(self.str.get('Jakes-Tik-Tok', 'Sorry unable to send message').format(author.id), reply=True, delete_after=60)
                                    else:
                                        if message.channel.id == 740717897948725358:
                                            return Response(self.str.get('Jakes-Tik-Tok', 'Sorry unable to send message').format(author.id), reply=True, delete_after=60)
                                        else:
                                            if message.channel.id == 740797019731394620:
                                                return Response(self.str.get('Jakes-Tik-Tok', 'Sorry unable to send message').format(author.id), reply=True, delete_after=60)
                                            else:
                                                if message.channel.id == 738988494789148702:
                                                    return Response(self.str.get('Jakes-Tik-Tok', 'Sorry unable to send message').format(author.id), reply=True, delete_after=60)
                                                else:
                                                    if message.channel.id == 739326860881559663:
                                                        return Response(self.str.get('Jakes-Tik-Tok', 'Sorry unable to send message').format(author.id), reply=True, delete_after=60)
                                                    else:
                                                        if message.channel.id == 739326860881559663:
                                                            return Response(self.str.get('Jakes-Tik-Tok', 'Sorry unable to send message').format(author.id), reply=True, delete_after=60)
                                                        else:
                                                            if message.channel.id == 738990727064846376:
                                                                return Response(self.str.get('Jakes-Tik-Tok', 'Sorry unable to send message').format(author.id), reply=True, delete_after=60)
                                                            else:
                                                                if message.channel.id == 738996443209007136:
                                                                    return Response(self.str.get('Jakes-Tik-Tok', 'Sorry unable to send message').format(author.id), reply=True, delete_after=60)
                                                                else:
                                                                    if message.channel.id == 738996508170649632:
                                                                        return Response(self.str.get('Jakes-Tik-Tok', 'Sorry unable to send message').format(author.id), reply=True, delete_after=60)
                                                                    else:
                                                                        if message.channel.id == 739730557042426008:
                                                                            return Response(self.str.get('Jakes-Tik-Tok', 'Sorry unable to send message').format(author.id), reply=True, delete_after=60)
                                                                        else:
                                                                            if message.channel.id == 739888012527272127:
                                                                                return Response(self.str.get('Jakes-Tik-Tok', 'Sorry unable to send message').format(author.id), reply=True, delete_after=60)
                                                                            else:
                                                                                if message.channel.id == 739908519364526111:
                                                                                    return Response(self.str.get('Jakes-Tik-Tok', 'Sorry unable to send message').format(author.id), reply=True, delete_after=60)
                                                                                else:
                                                                                    if message.channel.id == 738991422513872947:
                                                                                        return Response(self.str.get('Jakes-Tik-Tok', 'Sorry unable to send message').format(author.id), reply=True, delete_after=60)
                                                                                    else:
                                                                                        if message.channel.id == 738991474271584257:
                                                                                            return Response(self.str.get('Jakes-Tik-Tok', 'Sorry unable to send message').format(author.id), reply=True, delete_after=60)
                                                                                        else:
                                                                                            if message.channel.id == 738991508782317640:
                                                                                                return Response(self.str.get('Jakes-Tik-Tok', 'Sorry unable to send message').format(author.id), reply=True, delete_after=60)
                                                                                            else:
                                                                                                if message.channel.id == 738999749247631370:
                                                                                                    return Response(self.str.get('Jakes-Tik-Tok', 'Sorry unable to send message').format(author.id), reply=True, delete_after=60)
                                                                                                else:
                                                                                                    if message.channel.id == 739539537948049528:
                                                                                                        return Response(self.str.get('Jakes-Tik-Tok', 'Sorry unable to send message').format(author.id), reply=True, delete_after=60)
                                                                                                    else:
                                                                                                        if message.channel.id == 739165541565988894:
                                                                                                            return Response(self.str.get('Jakes-Tik-Tok', 'Sorry unable to send message').format(author.id), reply=True, delete_after=60)
                                                                                                        else:
                                                                                                            if message.channel.id == 739165389208027259:
                                                                                                                return Response(self.str.get('Jakes-Tik-Tok', 'Sorry unable to send message').format(author.id), reply=True, delete_after=60)
                                                                                                            else:
                                                                                                                if message.channel.id == 739165595949465662:
                                                                                                                    return Response(self.str.get('Jakes-Tik-Tok', 'Sorry unable to send message').format(author.id), reply=True, delete_after=60)
                                                                                                                else:
                                                                                                                    if message.channel.id == 739165708511871056:
                                                                                                                        return Response(self.str.get('Jakes-Tik-Tok', 'Sorry unable to send message').format(author.id), reply=True, delete_after=60)
                                                                                                                    else:
                                                                                                                        if message.channel.id == 739166025081290895:
                                                                                                                            return Response(self.str.get('Jakes-Tik-Tok', 'Sorry unable to send message').format(author.id), reply=True, delete_after=60)
                                                                                                                        else:
                                                                                                                            if message.channel.id == 739047115879219281:
                                                                                                                                return Response(self.str.get('Jakes-Tik-Tok', 'Sorry unable to send message').format(author.id), reply=True, delete_after=60)
                                                                                                                            else:
                                                                                                                                if message.channel.id == 739047353155321939:
                                                                                                                                    return Response(self.str.get('Jakes-Tik-Tok', 'Sorry unable to send message').format(author.id), reply=True, delete_after=60)
                                                                                                                                else:
                                                                                                                                    if message.channel.id == 739047515588001894:
                                                                                                                                        return Response(self.str.get('Jakes-Tik-Tok', 'Sorry unable to send message').format(author.id), reply=True, delete_after=60)
                                                                                                                                    else:
                                                                                                                                        if message.channel.id == 739047605920595990:
                                                                                                                                            return Response(self.str.get('Jakes-Tik-Tok', 'Sorry unable to send message').format(author.id), reply=True, delete_after=60)
                                                                                                                                        else:
                                                                                                                                            if message.channel.id == 739047677332946944:
                                                                                                                                                return Response(self.str.get('Jakes-Tik-Tok', 'Sorry unable to send message').format(author.id), reply=True, delete_after=60)
                                                                                                                                            else:
                                                                                                                                                if message.channel.id == 739047749504335872:
                                                                                                                                                    return Response(self.str.get('Jakes-Tik-Tok', 'Sorry unable to send message').format(author.id), reply=True, delete_after=60)
                                                                                                                                                else:
                                                                                                                                                    if message.channel.id == 739138427395768330:
                                                                                                                                                        return Response(self.str.get('Jakes-Tik-Tok', 'Sorry unable to send message').format(author.id), reply=True, delete_after=60)
                                                                                                                                                    else:
                                                                                                                                                        if message.channel.id == 739002179444015134:
                                                                                                                                                            return Response(self.str.get('Jakes-Tik-Tok', 'Sorry unable to send message').format(author.id), reply=True, delete_after=60)
                                                                                                                                                        else:
                                                                                                                                                            if message.channel.id == 739020856365416511:
                                                                                                                                                                return Response(self.str.get('Jakes-Tik-Tok', 'Sorry unable to send message').format(author.id), reply=True, delete_after=60)
                                                                                                                                                            else:
                                                                                                                                                                if message.channel.id == 740314452741718109:
                                                                                                                                                                    return Response(self.str.get('Jakes-Tik-Tok', 'Sorry unable to send message').format(author.id), reply=True, delete_after=60)
                                                                                                                                                                else:
                                                                                                                                                                    if message.channel.id == 739042486827221023:
                                                                                                                                                                        return Response(self.str.get('Jakes-Tik-Tok', 'Sorry unable to send message').format(author.id), reply=True, delete_after=60)
                                                                                                                                                                    else:
                                                                                                                                                                        if message.channel.id == 739042548177305661:
                                                                                                                                                                            return Response(self.str.get('Jakes-Tik-Tok', 'Sorry unable to send message').format(author.id), reply=True, delete_after=60)
                                                                                                                                                                        else:
                                                                                                                                                                            if message.channel.id == 739042588673441823:
                                                                                                                                                                                return Response(self.str.get('Jakes-Tik-Tok', 'Sorry unable to send message').format(author.id), reply=True, delete_after=60)
                                                                                                                                                                            else:
                                                                                                                                                                                if message.channel.id == 740791763223969854:
                                                                                                                                                                                    return Response(self.str.get('Jakes-Tik-Tok', 'Sorry unable to send message').format(author.id), reply=True, delete_after=60)
                                                                                                                                                                                else:
                                                                                                                                                                                    await self.safe_send_message(channel, "Unable To send Message To This Chat! This channel has disabled this command for safetly and privacy reasons!")

            

    async def cmd_daddy(self, channel):
        """
        Usage:
            {command_prefix}daddy

        sends random memes of jake
        """
        var = [
        "https://media1.tenor.com/images/1b177ba06e05dd16c1bd73d15315ea8e/tenor.gif?itemid=18014684 | Made by Diana",
        "https://media1.tenor.com/images/66737ba4d582ef5be18dbb0520a01869/tenor.gif?itemid=18014687 | Made by Diana",
        "https://media1.tenor.com/images/146b9d8b3a628fc03035c1ebe46b1b42/tenor.gif?itemid=18014694 | Made by Diana",
        "https://media1.tenor.com/images/2d118998209f7610c299feead4f24399/tenor.gif?itemid=18014704 | Made by Diana",
        "https://media1.tenor.com/images/5a41f1a9363588eaef548928435d5442/tenor.gif?itemid=18014715 | Made by Diana",
        "https://media1.tenor.com/images/6eacafccae730e7248e709592eb26ae4/tenor.gif?itemid=18042680 | Made by Diana",
        "https://media.discordapp.net/attachments/739326860881559663/739562199504257054/20200802_124801.jpg?width=465&height=468 | Made by Diana",
        "https://cdn.discordapp.com/attachments/739326860881559663/739334268316287126/20200801_221020.jpg | Made by Diana",
        "https://media1.tenor.com/images/15238934f05b5fc2ece177cfdfcfb7c6/tenor.gif?itemid=18042732 | Made by Diana",
        "https://media1.tenor.com/images/090e836aceb5079f3a387fdd1c0969e7/tenor.gif?itemid=18042703 | Made by Diana",
        "https://media1.tenor.com/images/f595cc3994e95588d51c680a4cfaaf81/tenor.gif?itemid=18042729 | Made by Diana",
        "https://media1.tenor.com/images/396ffee055c22fbc57b3172ca914478c/tenor.gif?itemid=18042713 | Made by Diana",
        "https://media.discordapp.net/attachments/739326860881559663/740097559153999912/image0.jpg",
        "https://cdn.discordapp.com/attachments/740607157132722176/740705347315957760/20200802_2125042.jpg",
        "https://cdn.discordapp.com/attachments/740979034539687936/740984001506050058/Adobe_20200805_194203.png",
        "https://media.discordapp.net/attachments/740979034539687936/741724786492833852/PicsArt_08-07-01.15.53.jpg?width=576&height=585",
        "https://media1.tenor.com/images/0f3bdd7618aac780918efc8b0abda817/tenor.gif?itemid=18042675",
        "https://media1.tenor.com/images/d8a28bc325597480f3a946ec7c538505/tenor.gif?itemid=18042681",
        "https://media1.tenor.com/images/bb35c7ff2bb91c2183df166b94868d23/tenor.gif?itemid=18042722",
        "https://media.discordapp.net/attachments/740979034539687936/742134099266764821/PicsArt_08-09-01.00.28.jpg?width=486&height=585",
        "https://media1.tenor.com/images/b2cc0551b5dea46ce950faaecc5e2cc2/tenor.gif?itemid=18042689",
        "https://cdn.discordapp.com/attachments/740979034539687936/740989240019779685/20200802_125147.png",
        "https://media1.tenor.com/images/9ea841043eeb54f39110dad00712bf5a/tenor.gif?itemid=18042697",
        "https://media.discordapp.net/attachments/740979034539687936/741370914049884320/PicsArt_08-06-11.21.18.jpg?width=327&height=585",
        "https://media.discordapp.net/attachments/739326860881559663/740809076023361596/Mematic_meme_20200805_003911.png", ]
        #how many jake memes do i need its like  21 alreaddyy just cmonnn
        await self.safe_send_message(channel, "{}".format(random.choice(var)), reply=True)

    async def cmd_support(self, channel, user_mentions, author):
        """
        Usage:
           {command_prefix}support

        sends a supportive message
        """
        var = [
        "You are doing an amazing job, keep it up",
        "Dont give you, i know you're trying your best, and you are doing amazing",
        "Never give up",
        "If you need any help, theres always people here to help you",
        "Everything is going to be okay",
        "Keep up the good work!", ]
        if not user_mentions:
            return Response(self.str.get('', '{}').format(random.choice(var)), reply=True, delete_after=60)

    async def cmd_respectjake(self, channel, message):
        """
        Usage:
           {command_prefix}respectjake

        This is so that you dont disrepsect jake, you can see what you should do and shouldnt
        """
        if message.channel.id == 738977422581694607:
            await self.safe_send_message(channel, "Respect Jakes pronouns as theres no point in disrespecting him like that, and if you do disrepsect him on **purpose** just leave there is no point of staying and being toxic")
        else: 
            if message.channel.id == 739163145901310013:
                await self.safe_send_message(channel, "Respect Jakes pronouns as theres no point in disrespecting him like that, and if you do disrepsect him on **purpose** just leave there is no point of staying and being toxic")
            else:
                if message.channel.id == 738981082921173085:
                    ...
                    if message.channel.id == 728696428997640265:
                        ...
                        if message.channel.id == 739157372362293329:
                            await self.safe_send_message(channel, "Respect Jakes pronouns as theres no point in disrespecting him like that, and if you do disrepsect him on **purpose** just leave there is no point of staying and being toxic")
                            #HeH just to keep jake safe, now copy it 100 more timews and then kill myself
                        else: 
                            if message.channel.id == 739022711317200948:
                                await self.safe_send_message(channel, "Respect Jakes pronouns as theres no point in disrespecting him like that, and if you do disrepsect him on **purpose** just leave there is no point of staying and being toxic")
                            else:
                                                                                                                                                                        ...
                                                                                                                                                                        if message.channel.id == 739042548177305661:
                                                                                                                                                                            ...
                                                                                                                                                                            if message.channel.id == 739042588673441823:
                                                                                                                                                                                ...
                                                                                                                                                                                if message.channel.id == 740791763223969854:
                                                                                                                                                                                    ...
                                                                                                                                                                                    await self.safe_send_message(channel, "Unable To send Message To This Chat! This channel has disabled this command for safetly and privacy reasons!")
                                                                                                                                                                                    #Somehow i survided, well this is all for today folkes *looks at watch* damn 6:42am!

    async def cmd_version(self, message, channel):
        """
        Usage
            {command_prefix}version

        Sends the version that GoodDragon!
        """
        await self.safe_send_message(channel, "The latest version is: {}, in this version multiple bugs have been fixed and commands have been added!".format(BOTVERSION))

    async def cmd_praise(self, message, channel):
        """
        Usage:
            {command_prefix}praise

        Sends a message that the bot is proud of you!
        """
        #Lix
        var = [
        "you look nice today",
        ...
        return Response(self.str.get('', '{}').format(random.choice(var)))

    async def cmd_recommend(self, message, channel, command=None):
        """
        Usage:
            {command_prefix}recommend (anime, music)

        Recommends a topic if inputed a second arg
        """
        if command:
            if command.lower() == 'anime':
                await self.safe_send_message(channel, '(not in any specific order!) \n1) Death note \n2) My Hero Academia \n3) Haikyu \n4) Naruto \n5) Toradora \n6) Demon Slayer \n7) Seven Deadly Sins \n8) FullMetal Alchemist \n9) Attack on Titan \n10) Saiki K \nreact to ▶️ to get to the next page!')
                await message.add_reaction("▶️")
                    #pls kill me i dont want to do this anymore!
                if message.reaction.emoji == "▶️":
                    if message.reaction.count == 2:
                        await self.safe_send_message(channel, 'Page 2! (again no specific order) \n11) Your Lie in April')

        else:
            await self.safe_send_message(channel, 'Sorry, not enough Args!')

    async def cmd_backflip(self, message, channel):
        """
        Usage:
            {command_prefix}backflip

        Backflip
        """
        await self.safe_send_message(channel, '**Epic Backflip**')

    async def cmd_muffin(self, message, channel):
        """
        Usage:
            {command_prefix}muffin

        Muffin
        """
        await self.safe_send_message(channel, 'Muffin 🧁')

    async def cmd_cake(self, message, channel):
        """
        Usage:
            {command_prefix}cake
        ...
        Gives you cake
        """
        await self.safe_send_message(channel, 'Surprise here is your cake :cake:')

    async def cmd_soup(self, message, channel, user_mentions, author):
        """
        Usage:
            {command_prefix}soup
        ...
        Give you soup
        """
        if message.author.id == 372099126583754753:
            await self.safe_send_message(channel, 'Has given soup to Hamburbur! :ramen: ')
        else:
            if message.author.id == 368966303584878593:
                await self.safe_send_message(channel, 'Has given soup to Mc! :ramen:')
            else:
                if message.author.id == 412034769820581899:
                    await self.safe_send_message(channel, 'Has given soup to Octo! :ramen:')
                else:
                    if message.author.id == 673001979412217857:
                        await self.safe_send_message(channel, 'Has given soup to 🅳🅸🅰🅼🅾🅽🅳 🆂🆃🅴🆅🅴🅽! :ramen:')
                    else:
                        if message.author.id == 623984134288572416:
                            await self.safe_send_message(channel, 'Has given soup to River! :ramen:')
                        else:
                            if message.author.id == 742821362229313536:
                                await self.safe_send_message(channel, 'Has given soup to tetramask223! :ramen:')
                            else:
                                if message.author.id == 506635578662715432:
                                    await self.safe_send_message(channel, 'Has given soup to Jake!! :ramen:')
                                else:
                                    if message.author.id == 690630972923838626:
                                        await self.safe_send_message(channel, 'Has given soup to Zeno! :ramen:')
                                    else:
                                        if message.author.id == 461861819758149632:
                                            await self.safe_send_message(channel, 'Has given soup to Lix! :ramen:')
                                        else:
                                            if message.author.id == 528694844965650433:
                                                await self.safe_send_message(channel, 'Has given soup to Jaden2x! :ramen:')
                                            else:
                                                if message.author.id == 735088886664921128:
                                                    await self.safe_send_message(channel, 'Has given soup to Ezrial! :ramen:')
                                                else:
                                                    await self.safe_send_message(channel, '**Error!**: The soup command has been discontinued becuase of technical issues')
